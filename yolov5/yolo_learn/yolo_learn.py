...
def main_crop(ini, logger=None):
    ...
    raw_fnames = sorted(
        g_utils.get_filenames(raw_path, extensions=g_utils.IMG_EXTENSIONS)
    )
    ann_fnames = sorted(
        g_utils.get_filenames(ann_path, extensions=g_utils.META_EXTENSION)
    )
    logger.info(
        " [CROP] # Total file number to be processed: {:d}.".format(len(raw_fnames))
    )

    for idx, raw_fname in enumerate(raw_fnames):
        logger.info(
            " [CROP] # Processing {} ({:d}/{:d})".format(
        ...
        for obj in objects:
            ...
            rst_fpath = os.path.join(
                _project_folder_,
                ini["img_path"] + raw_core_name + "_" + str(object_cnt) + raw_ext,
            )
...
def main_generate(ini, logger=None):
    ...
    logger.info(
        " [GENERATE] # Total file number to be processed: {:d}.".format(len(raw_fnames))
    )

    for idx, raw_fname in enumerate(raw_fnames):
        logger.info(
            " [GENERATE] # Processing {} ({:d}/{:d})".format(
                raw_fname, (idx + 1), len(raw_fnames)
            )
        )
        ...
        for obj in objects:
            ...
            rst_fpath = os.path.join(
                _project_folder_, ini["label_path"] + raw_core_name + ".txt"
            )
...
def main_split(ini, logger=None):
    ...
    if g_utils.file_exists(ini["train_path"]):
        print(
            " @ Warning: train text file path, {}, already exists".format(
                ini["train_path"]
            )
        )
    ...
    if g_utils.file_exists(ini["val_path"]):
        print(
            " @ Warning: test text file path, {}, already exists".format(
                ini["val_path"]
            )
        )
        ans = input(" % Proceed (y/n) ? ")
    ...
    img_fnames = sorted(
        g_utils.get_filenames(img_path, extensions=g_utils.IMG_EXTENSIONS)
    )
    if len(img_fnames) == 0:
    ...
    train_img_list, test_img_list = train_test_split(
        img_fnames, test_size=test_ratio, random_state=2000
    )
    # Save train.txt file
    ...
    logger.info(
        " [SPLIT] # Train : Test ratio -> {} : {}".format(train_ratio, test_ratio)
    )
    logger.info(
        " [SPLIT] # Train : Test size  -> {} : {}".format(
...
def main_train_test(ini, logger=None):
    ...
    subprocess.call(
        train_cmd, shell=True, cwd=ini["train_root_path"]
    )  # train by python
...
def main(args):
    ...
    logger = g_utils.setup_logger_with_ini(
...
def parse_arguments(argv):
    ...
    parser.add_argument(
        "--target_mode", required=True, choices=["PROBLEM", "GRAPH"], help="Target mode"
    )
    parser.add_argument(
        "--op_mode",
        required=True,
        choices=["CROP", "GENERATE", "SPLIT", "TRAIN_TEST"],
        help="Operation mode",
    )
