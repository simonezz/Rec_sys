...
from supervisely_lib.annotation.tag import Tag, TagValueType
...
class PointTest(unittest.TestCase):
    def setUp(self):
        ...
        self._confidence_tag_meta = TagMeta(
            name="confidence", value_type=TagValueType.ANY_NUMBER
        )
        ...
        self._gt_obj_1 = Label(
            obj_class=self._obj_class_gt, geometry=Rectangle(0, 0, 10, 10)
        )

        # Will match self._pred_obj_3
        self._gt_obj_2 = Label(
            obj_class=self._obj_class_gt, geometry=Rectangle(13, 13, 15, 15)
        )

        # Will be a false negative
        self._gt_obj_3 = Label(
            obj_class=self._obj_class_gt, geometry=Rectangle(43, 43, 45, 45)
        )
        ...
        self._pred_objs_fp = [
            Label(
                ...
                tags=TagCollection(
                    [Tag(meta=self._confidence_tag_meta, value=v / 100)]
                ),
            )
    ...
    def test_empty_gt(self):
        ann = Annotation(
            img_size=[100, 100],
            labels=[self._pred_obj_1, self._pred_obj_2, self._pred_obj_3]
            + self._pred_objs_fp,
        )
