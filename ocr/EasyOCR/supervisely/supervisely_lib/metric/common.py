...
def check_class_mapping(
    first_project: Project, second_project: Project, classes_mapping: dict
) -> None:
    for k, v in classes_mapping.items():
        if first_project.meta.obj_classes.get(k) is None:
            raise RuntimeError(
                'Class {} does not exist in input project "{}".'.format(
                    k, first_project.name
                )
            )
        if second_project.meta.obj_classes.get(v) is None:
            raise RuntimeError(
                'Class {} does not exist in input project "{}".'.format(
                    v, second_project.name
                )
            )


def check_tag_mapping(
    first_project: Project, second_project: Project, tags_mapping: dict
) -> None:
    for k, v in tags_mapping.items():
        if not first_project.meta.tag_metas.has_key(k):
            raise RuntimeError(
                'Tag {} does not exist in input project "{}".'.format(
                    k, first_project.name
                )
            )
...
def render_labels_for_class_name(labels, class_name, canvas):
    return render_labels_for_classes(
        labels, {class_name: True}, canvas, missing_classes_color=False
    )
...
def sum_counters(elementwise_counters, counter_names):
    return {
        counter_name: sum(c.get(counter_name, 0) for c in elementwise_counters)
