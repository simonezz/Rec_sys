...
LabelsMatchResult = namedtuple(
    "LabelsMatchResult", ["matches", "unmatched_labels_1", "unmatched_labels_2"]
)
IndexPairWithScore = namedtuple("IndexPairWithScore", ["idx_1", "idx_2", "score"])
IndexMatchResult = namedtuple(
    "IndexMatchResult", ["matches", "unmatched_indices_1", "unmatched_indices_2"]
)
...
def match_indices_by_score(elems_1, elems_2, score_threshold, score_fn):
    ...
    scored_idx_pairs = [
        ...
        for idx_1, elem_1 in enumerate(elems_1)
        for idx_2, elem_2 in enumerate(elems_2)
    ]
    ...
    sorted_idx_pairs = sorted(
        thresholded_idx_pairs, key=lambda p: p.score, reverse=True
    )
    ...
    return IndexMatchResult(
        matches=matches,
        unmatched_indices_1=unmatched_1,
        unmatched_indices_2=unmatched_2,
    )


def match_labels_by_iou(labels_1, labels_2, img_size, iou_threshold):
    index_matches = match_indices_by_score(
        labels_1, labels_2, iou_threshold, score_fn=get_labels_iou
    )
    return LabelsMatchResult(
        matches=[
            LabelsPairWithScore(
                label_1=labels_1[match.idx_1],
                label_2=labels_2[match.idx_2],
                score=match.score,
            )
