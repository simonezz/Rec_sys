...
class ClassificationMetrics(MetricsBase):
    def __init__(self, tags_mapping, confidence_threshold=0):
        ...
        self._counters = {
            tag_name_gt: {counter: 0 for counter in RAW_COUNTERS}
            for tag_name_gt in self._tags_mapping.keys()
        }
    ...
    def _classification_metrics(self, ann_1, ann_2):
        def is_passes_confidence_threshold(tag):
            if tag.meta.value_type == TagValueType.NONE:
                ...
                tag.meta.value_type == TagValueType.ANY_STRING
                or tag.meta.value_type == TagValueType.ONEOF_STRING
            ):
    ...
    def log_total_metrics(self):
        ...
        for i, (tag_name_gt, tag_metrics) in enumerate(
            self.get_metrics().items(), start=1
        ):
            logger.info(
                "{}) {} <--> {}:".format(
