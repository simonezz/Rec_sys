...
class PrecisionRecallMetric(MetricsBase):
    def __init__(self, class_mapping, iou_threshold):
        ...
        self._counters = {
            gt_cls: {counter: 0 for counter in RAW_COUNTERS}
    ...
    def add_pair(self, ann_gt, ann_pred):
        for key in self._gt_to_pred_class_mapping.keys():
            ...
            labels_pred = filter_labels_by_name(
                ann_pred.labels, [self._gt_to_pred_class_mapping[key]]
            )
            ...
            for label_pred in labels_pred:
                self._counters[
                    self._pred_to_gt_class_mapping[label_pred.obj_class.name]
                ][TOTAL_PREDICTIONS] += 1

    @staticmethod
    def _compute_composite_metrics(metrics_dict):
        metrics_dict[PRECISION] = safe_ratio(
            metrics_dict[TRUE_POSITIVE], metrics_dict[TOTAL_PREDICTIONS]
        )
        metrics_dict[RECALL] = safe_ratio(
            metrics_dict[TRUE_POSITIVE], metrics_dict[TOTAL_GROUND_TRUTH]
        )
