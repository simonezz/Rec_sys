...
class MAPMetric(MetricsBase):
    def __init__(
        ...
        self._pred_to_gt_class_mapping = {
            pred_name: gt_name
            for gt_name, pred_name in self._gt_to_pred_class_mapping.items()
        }
        if len(self._pred_to_gt_class_mapping) != len(self._gt_to_pred_class_mapping):
            raise ValueError(
                "Mapping from ground truth to prediction class name is not 1-to-1: multiple ground truth "
        ...
        self._counters = {
            gt_cls: {MATCHES: [], TOTAL_GROUND_TRUTH: 0}
    ...
    def add_pair(self, ann_gt, ann_pred):
        ...
        all_labels_pred = [
            label
            for label in filter_labels_by_name(
                ann_pred.labels, self._pred_to_gt_class_mapping
            )
        ]
        labels_pred = []
        for label in all_labels_pred:
            ...
            if label_confidence is None:
                logger.warn(
                    f"Found a label with class {label.obj_class.name!r} that does not have a "
        ...
        match_result = match_labels_by_iou(
            labels_1=labels_gt,
            labels_2=labels_pred,
        ...
        for umatched_pred in match_result.unmatched_labels_2:
            ...
            self._counters[gt_class][MATCHES].append(
                MatchWithConfidence(
                    is_correct=False,
                    confidence=self._get_confidence_value(umatched_pred),
                )
            )
    ...
    def _calculate_average_precision(gt_class, pred_class, pair_counters):
        ...
        sorted_matches = sorted(
            pair_counters[MATCHES], key=lambda match: match.confidence, reverse=True
        )
        ...
        for anchor_recall in np.linspace(0, 1, 11):
            ...
            anchor_precisions.append(
                np.max(precisions[points_above_recall])
                if np.any(points_above_recall)
                else 0
            )
        ...
        result = {
            gt_class: {
                AP: self._calculate_average_precision(
                    gt_class, self._gt_to_pred_class_mapping[gt_class], pair_counters
                )
            }
    ...
    def average_per_class_avg_precision(per_class_metrics):
        return np.mean(
            [class_metrics[AP] for class_metrics in per_class_metrics.values()]
        )
    ...
    def log_total_metrics(self):
        log_line()
        log_head(
            " Result metrics values for {} IoU threshold ".format(self._iou_threshold)
        )
        ...
        for i, (cls_gt, pair_values) in enumerate(classes_values.items()):
            ...
            log_line()
            log_head(
                " Results for pair of classes <<{} <-> {}>>  ".format(
                    cls_gt, self._gt_to_pred_class_mapping[cls_gt]
                )
            )
        ...
        logger.info(
            "Mean Average Precision (mAP): {}".format(self.get_total_metrics()[AP])
        )
        log_line()
