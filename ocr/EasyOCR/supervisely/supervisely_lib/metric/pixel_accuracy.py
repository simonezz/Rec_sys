...
from supervisely_lib.metric.common import (
    render_labels_for_classes,
    safe_ratio,
    TRUE_POSITIVE,
    TRUE_NEGATIVE,
    FALSE_POSITIVE,
...
class PixelAccuracyMetric(MetricsBase):
    def __init__(self, class_mapping):
        ...
        self._gt_colors = {}
        self._pred_colors = {}
        for class_idx, (gt_class, pred_class) in enumerate(
            self._class_mapping.items(), start=1
        ):
    ...
    def add_pair(self, ann_gt, ann_pred):
        ...
        render_gt, render_pred = np.zeros(img_size, dtype=np.int32), np.zeros(
            img_size, dtype=np.int32
        )
        render_labels_for_classes(
            ann_gt.labels, self._gt_colors, render_gt, missing_classes_color=0
        )
        render_labels_for_classes(
            ann_pred.labels, self._pred_colors, render_pred, missing_classes_color=0
        )
        ...
        for cls_gt, cls_pred in self._class_mapping.items():
            ...
            class_pair_counters[TRUE_NEGATIVE] += (
                np.logical_not(mask_gt) & np.logical_not(mask_pred)
            ).sum()
            class_pair_counters[FALSE_POSITIVE] += (
                np.logical_not(mask_gt) & mask_pred
            ).sum()
            class_pair_counters[FALSE_NEGATIVE] += (
                mask_gt & np.logical_not(mask_pred)
            ).sum()
            class_pair_counters[TOTAL] += mask_gt.size

        self._total_counters[TRUE_POSITIVE] += (
            (render_gt == render_pred) & (render_pred != 0)
        ).sum()
        self._total_counters[TRUE_NEGATIVE] += (
            (render_gt == 0) & (render_pred == 0)
        ).sum()
        self._total_counters[FALSE_POSITIVE] += (
            (render_gt != render_pred) & (render_pred != 0)
        ).sum()
        self._total_counters[FALSE_NEGATIVE] += (
            (render_gt != 0) & (render_pred == 0)
        ).sum()
    ...
    def _metrics_from_counters(counters):
        return {
            ...
            ACCURACY: safe_ratio(
                counters[TRUE_POSITIVE] + counters[TRUE_NEGATIVE], counters[TOTAL]
            ),
        }

    def get_metrics(self):
        return {
            cls_gt: self._metrics_from_counters(class_counters)
