...
class MetricProjectsApplier:
    def __init__(self, metric: MetricsBase, config: dict):
        ...
        self._project_pred = (
            self._get_project_or_die(PROJECT_PRED)
            if (PROJECT_PRED in self._confg)
            else self._project_gt
        )
    ...
    def _get_project_or_die(self, project_field_name):
        ...
        if project_name is None:
            raise RuntimeError(
                'Field "{}" does not exist in config.'.format(project_field_name)
            )
        return Project(os.path.join(TaskPaths.DATA_DIR, project_name), OpenMode.READ)

    def _check_projects_compatible_structure(self):
        if (
            self._project_gt.datasets.keys() != self._project_pred.datasets.keys()
        ):  # Keys is sorted - ok
        ...
        for ds_gt in self._project_gt.datasets:
            ...
            for sample_name in ds_gt:
                if not ds_pred.item_exists(sample_name):
                    raise RuntimeError(
                        "Projects must contain identically named samples in respective datasets. "
                        + "Ground truth project has sample {!r} in dataset {!r}, but prediction project "
                        + "does not.".format(sample_name, ds_gt.name)
                    )

        logger.info(
            "Projects structure has been read. Samples: {} per project.".format(
                self._project_gt.total_items
            )
        )
    ...
    def run_evaluation(self):
        ...
        for ds_name in self._project_gt.datasets.keys():
            ...
            for sample_name in ds_gt:
                try:
                    ann_gt = Annotation.load_json_file(
                        ds_gt.get_ann_path(sample_name), self._project_gt.meta
                    )
                    ann_pred = Annotation.load_json_file(
                        ds_pred.get_ann_path(sample_name), self._project_pred.meta
                    )
                    self._metric.add_pair(ann_gt, ann_pred)
                except ValueError as e:
                    logger.warning(
                        'An error has occured ({}). Sample "{}" in dataset "{}" will be skipped'.format(
                            str(e), sample_name, ds_gt.name
                        )
                    )
