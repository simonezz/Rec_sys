...
def _iou_log_line(iou, intersection, union):
    return "IoU = {:.6f},  mean intersection = {:.6f}, mean union = {:.6f}".format(
...
class IoUMetric(MetricsBase):
    def __init__(self, class_mapping):
        ...
        self._counters = {
            cls_gt: {INTERSECTION: 0, UNION: 0, TOTAL_GROUND_TRUTH: 0}
    ...
    def get_metrics(self):
        return {
            cls_gt: {
                INTERSECTION: safe_ratio(
                    class_counters[INTERSECTION], class_counters[TOTAL_GROUND_TRUTH]
                ),
                UNION: safe_ratio(
                    class_counters[UNION], class_counters[TOTAL_GROUND_TRUTH]
                ),
    ...
    def log_total_metrics(self):
        ...
        for i, (cls_gt, values) in enumerate(self.get_metrics().items(), start=1):
            ...
            logger.info(
                "{}. Classes {} <-> {}:   {}".format(
                    i, cls_gt, self._class_mapping[cls_gt], iou_line
                )
            )
        ...
        logger.info(
            "Total:   {}".format(
                _iou_log_line(
                    total_values[IOU], total_values[INTERSECTION], total_values[UNION]
                )
            )
        )
