...
from supervisely_lib.io.network_exceptions import (
...
class AgentAPI:
    ...
    def _send_request(
        self, api_method_name, request_data, timeout, in_stream, addit_headers
    ):
            ...
            with self._send_request(
                ...
                for buffer in reply.iter_content(chunk_size=None):
                    while len(buffer) > 0:
                        if msg_len is None:
                            if len(msg_buf) < 4:
                                msg_buf, buffer = append_to_msg_buffer(
                                    4, msg_buf, buffer
                                )
                            if len(msg_buf) < 4:
                                continue
    ...
    def get_endless_stream(
        ...
        for attempt in range(server_fail_limit):
            ...
            self.logger.warn(
                "Endless input stream end", extra={"method": api_method_name}
            )
