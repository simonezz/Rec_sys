...
from supervisely_lib.nn.hosted.inference_modes import (
...
from supervisely_lib.worker_api.agent_rpc import (
    decode_image,
...
class AgentRPCServicerBase:
    ...
    def __init__(
        self, logger, model_applier: SingleImageInferenceInterface, conn_config, cache
    ):
        self.logger = logger
        self.server_address = conn_config["server_address"]
        self.api = AgentAPI(
            token=conn_config["token"],
        ...
        try:
            ...
            if request_type == INFERENCE:
                ...
                if frame_info is None:
                    ...
                    self.logger.trace(
                        "Input image is obtained.", extra={REQUEST_ID: req_id}
                    )
                else:
                    # download frame
                    ...
                    image_uniq_key = "video_{}_frame_{}.png".format(
                    ...
                    if img_data is None:
                        ...
                        img_data = public_api.video.frame.download_np(
                            video_id, frame_index
                        )
            ...
            res_msg.update(
                {"success": False, "error": json.dumps(traceback.format_exc())}
            )
            self.thread_pool.submit(
                function_wrapper_nofail, self._send_data, res_msg, req_id
            )  # skip errors
    ...
    def _final_processing(self, in_msg):
        ...
        if request_type == INFERENCE:
            ...
            raise RuntimeError(
                "Unknown request type: {}. Only the following request types are supported: {}".format(
    ...
    def run_inf_loop(self):
        ...
        for gen_event in self.api.get_endless_stream(
            ...
            try:
                ...
                self.logger.warning(
                    "Inference exception: ", extra={"error_message": str(error)}
                )
                res_msg = {"success": False, "error": json.dumps(str(error))}
                self.thread_pool.submit(
                    function_wrapper_nofail, self._send_data, res_msg, request_id
                )
...
class AgentRPCServicer(AgentRPCServicerBase):
    ...
    def _make_inference_mode(self, msg_inference_mode_config, in_project_meta):
        ...
        return InferenceModeFactory.create(
            inference_mode_config, in_project_meta, self.model_applier
        )
    ...
    def _do_single_img_inference(self, img, in_msg):
        ...
        if ann_json is not None:
            if in_project_meta is None:
                raise ValueError(
                    "In order to perform inference with annotation you must specify the appropriate"
                    " project meta."
                )
        ...
        inference_mode = self._make_inference_mode(
            in_msg.get(MODE, {}), in_project_meta
        )
...
class InactiveRPCServicer(AgentRPCServicer):
    def __init__(
        ...
        self.logger.info(
            "Created InactiveRPCServicer for internal usage", extra=conn_config
        )
