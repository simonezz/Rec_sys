...
class PointcloudDataset(VideoDataset):
    ...
    def _validate_added_item_or_die(item_path):
        ...
        try:
            ...
            sly_pointcloud.UnsupportedPointcloudFormat,
            sly_pointcloud.PointcloudReadException,
        ):
            os.remove(item_path)
            raise
    ...
    def get_related_images_path(self, item_name):
        ...
        rimg_dir = os.path.join(
            self.directory, self.related_images_dir_name, item_name_temp
        )
        return rimg_dir
        ...
        if dir_exists(path):
            ...
            for file in files:
                img_meta_path = os.path.join(
                    path, get_file_name_with_ext(file) + ".json"
                )
                img_meta = {}
    ...
    for dataset in datasets_infos:
        ...
        if log_progress:
            ds_progress = Progress(
                "Downloading dataset: {!r}".format(dataset.name),
