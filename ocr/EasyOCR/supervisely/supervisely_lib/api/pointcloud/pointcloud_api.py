...
from supervisely_lib.api.pointcloud.pointcloud_annotation_api import (
...
class PointcloudApi(RemoveableBulkModuleApi):
    ...
    def _convert_json_info(self, info: dict, skip_missing=True):
        return super(PointcloudApi, self)._convert_json_info(
    ...
    def _download(self, id, is_stream=False):
        ...
        response = self._api.post(
            "point-clouds.download", {ApiField.ID: id}, stream=is_stream
        )
        return response
    ...
    def download_related_image(self, id, path):
        response = self._api.post(
            "point-clouds.images.download", {ApiField.ID: id}, stream=True
        )
        ensure_base_path(path)
        with open(path, "wb") as fd:
            for chunk in response.iter_content(chunk_size=1024 * 1024):
    ...
    def add_related_images(self, images_json):
        response = self._api.post(
            "point-clouds.images.add", {ApiField.IMAGES: images_json}
        )
