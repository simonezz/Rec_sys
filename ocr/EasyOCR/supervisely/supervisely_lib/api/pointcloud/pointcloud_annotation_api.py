...
from supervisely_lib.pointcloud_annotation.pointcloud_object_collection import (
...
class PointcloudAnnotationAPI(EntityAnnotationAPI):
    ...
    def append(
        self, pointcloud_id, ann: PointcloudAnnotation, key_id_map: KeyIdMap = None
    ):
        info = self._api.pointcloud.get_info_by_id(pointcloud_id)
        ...
        for object_3d in ann.objects:
            if (
                key_id_map is not None
                and key_id_map.get_object_id(object_3d.key()) is not None
            ):
