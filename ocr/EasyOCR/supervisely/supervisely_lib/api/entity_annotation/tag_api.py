...
class TagApi(ModuleApi):
    ...
    def _tags_to_json(
        self, tags: KeyIndexedCollection, tag_name_id_map=None, project_id=None
    ):
    ...
    def _append_json(self, entity_id, tags_json):
        ...
        response = self._api.post(
            ...
            {self._entity_id_field: entity_id, ApiField.TAGS: tags_json},
        )
        ids = [obj[ApiField.ID] for obj in response.json()]
        return ids

    def append_to_objects(
        self, entity_id, project_id, objects: KeyIndexedCollection, key_id_map: KeyIdMap
    ):
        tag_name_id_map = self.get_name_to_id_map(project_id)
