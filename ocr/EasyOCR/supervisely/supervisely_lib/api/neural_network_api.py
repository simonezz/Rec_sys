...
from supervisely_lib.api.module_api import (
    ApiField,
    CloneableModuleApi,
    RemoveableModuleApi,
)
...
class NeuralNetworkApi(CloneableModuleApi, RemoveableModuleApi):
    ...
    def get_output_meta(self, id, input_meta=None, inference_mode=None):
        ...
        encoder = MultipartEncoder(
            {"id": str(id).encode("utf-8"), "data": json.dumps(data)}
        )
        response = self._api.post("models.infer", MultipartEncoderMonitor(encoder))
    ...
    def get_training_metrics(self, model_id):
        response = self._get_response_by_id(
            id=model_id, method="tasks.train-metrics", id_field=ApiField.MODEL_ID
        )
    ...
    def create_from_checkpoint(
        self, task_id, checkpoint_id, model_name, change_name_if_conflict=True
    ):
        ...
        if new_model_name != model_name and change_name_if_conflict is False:
            raise KeyError(
                "Model name={!r} already exists in workspace id={!r}".format(
                    model_name, workspace_id
                )
            )
        resp = self._api.post(
            "models.create-from-checkpoint",
            {
                ApiField.ID: checkpoint_id,
