...
class ReportApi(ModuleApiBase):
    ...
    def create(self, team_id, name, widgets, layout=""):
        response = self._api.post(
            "reports.create",
            {ApiField.TEAM_ID: team_id, ApiField.NAME: name, ApiField.WIDGETS: widgets},
        )
    ...
    def create_notification(
        self, name, content, notification_type=NotificationType.INFO
    ):
        return {
            "type": str(WidgetType.NOTIFICATION),
            "title": name,
            "content": content,
            "options": {"type": str(notification_type)},
        }
