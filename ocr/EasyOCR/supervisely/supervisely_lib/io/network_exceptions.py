...
def process_requests_exception(
    ...
    if (
        ...
        try:
            ...
            is_need_ping_error = (
                server_explanation.get("error", None) == SPECIAL_RECONNECT_ERROR
            )
...
def process_retryable_request(
    ...
    if retry_info is not None:
        ...
        user_message = "{}:  Retrying ({}/{}).".format(
            user_message, retry_idx, retry_limit
        )
    if verbose:
        external_logger.warn(
            user_message,
            extra={"method": api_method_name, "url": url, "details": str(exc)},
        )
...
def process_unhandled_request(external_logger, exc):
    ...
    external_logger.error(
        traceback.format_exc(), exc_info=True, extra={"exc_str": exc_str}
    )
