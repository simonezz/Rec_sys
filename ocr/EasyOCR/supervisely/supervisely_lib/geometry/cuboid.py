...
class Cuboid(Geometry):
    ...
    def __init__(
        ...
        if len(faces) != 3:
            raise ValueError(
                f"A cuboid must have exactly 3 faces. Instead got {len(faces)} faces."
            )
    ...
    def _draw_contour_impl(self, bitmap, color, thickness=1, config=None):
        contours_np_list = [
            np.array(contour, dtype=np.int32) for contour in self._contours_list()
        ]
        cv2.polylines(
            bitmap,
            pts=contours_np_list,
            isClosed=True,
            color=color,
            thickness=thickness,
        )
