...
def _is_2d_typed_coords_valid(coords, individual_coord_types):
    return isinstance(coords, (list, tuple)) and all(
        len(point) == 2
        and all(isinstance(coord, individual_coord_types) for coord in point)
        for point in coords
    )
...
def validate_geometry_points_fields(json_obj: dict) -> None:
    ...
    if EXTERIOR not in points_obj or INTERIOR not in points_obj:
        raise ValueError(
            '"{}" field must contain {} and {} fields.'.format(
    ...
    if not isinstance(interior, list) or not all(
        _is_2d_numeric_coords_valid(interior_component)
        for interior_component in interior
    ):
        raise TypeError(
            "{} field must be a list of lists of 2 numbers lists.".format(INTERIOR)
        )
