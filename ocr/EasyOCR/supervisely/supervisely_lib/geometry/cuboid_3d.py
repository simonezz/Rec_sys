...
class Cuboid3d(Geometry):
    ...
    def __init__(
        ...
        if type(position) is not Vector3d:
            raise TypeError(
                '"position" param has to be of type {!r}'.format(type(Vector3d))
            )
        if type(rotation) is not Vector3d:
            raise TypeError(
                '"rotation" param has to be of type {!r}'.format(type(Vector3d))
            )
        if type(dimensions) is not Vector3d:
            raise TypeError(
                '"dimensions" param has to be of type {!r}'.format(type(Vector3d))
            )
