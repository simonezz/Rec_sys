...
class VectorGeometry(Geometry):
    ...
    def __init__(
        ...
        if not (
            ...
            and all(isinstance(p, PointLocation) for p in exterior)
        ):
            raise TypeError(
        ...
        if (
            ...
            or not all(isinstance(c, list) for c in interior)
            or not all(isinstance(p, PointLocation) for c in interior for p in c)
        ):
            raise TypeError(
                'Argument "interior" must be list of list of "PointLocation" objects!'
            )
