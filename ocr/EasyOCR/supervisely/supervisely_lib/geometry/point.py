...
class Point(Geometry):
    ...
    def crop(self, rect):
        ...
        """
        return (
            [self.clone()] if rect.contains_point_location(self.point_location) else []
        )
    ...
    def _draw_impl(self, bitmap, color, thickness=1, config=None):
        r = round(
            thickness / 2
        )  # @TODO: relation between thickness and point radius - ???
        cv2.circle(
            bitmap, (self.col, self.row), radius=r, color=color, thickness=cv2.FILLED
        )
