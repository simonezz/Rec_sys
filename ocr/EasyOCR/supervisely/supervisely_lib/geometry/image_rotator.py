...
class ImageRotator:
    ...
    def _affine_matrix_and_new_canvas_size(imsize, angle_degrees_ccw):
        ...
        new_canvas_size = (
            np.ceil(rotated_upper_values - rotated_lower_values + 1)
            .astype(np.int64)
            .tolist()
        )
    ...
    def transform_point(self, point):
        ...
        return PointLocation(
            row=round(transformed_np[0].item()), col=round(transformed_np[1].item())
        )
    ...
    def rotate_img(self, img, use_inter_nearest):
        ...
        if use_inter_nearest:
            interp = (
                cv2.INTER_NEAREST
            )  # @TODO: cv2 INTER_NEAREST may shift coords, what to do?
        else:
            interp = cv2.INTER_LANCZOS4
        res = cv2.warpAffine(
            src=img,
            M=self.opencv_affine_matrix,
            dsize=tuple(self.new_imsize[::-1]),
            flags=interp,
        )
        return res
