...
from supervisely_lib.imaging.image import (
...
def resize_origin_and_bitmap(
    origin: PointLocation, bitmap: np.ndarray, in_size, out_size
):
    """
...
class BitmapBase(Geometry):
    ...
    def from_json(cls, json_data):
        ...
        if (
            ORIGIN not in json_data[json_root_key]
            or DATA not in json_data[json_root_key]
        ):
            raise ValueError(
    ...
    def _resize_mask(mask, out_rows, out_cols):
        return resize_inter_nearest(mask.astype(np.uint8), (out_rows, out_cols)).astype(
            np.bool
        )
