...
def is_not_started_desc():
    return (
        "PENDING",
        "the number of jobs labeler haven't even opened yet (created but not started)",
    )
...
def is_zero_labeling_desc():
    return (
        "ZERO LABELED",
        'the number of jobs with status "IN PROGRESS" with zero labeled items',
    )
...
def is_zero_reviewed_desc():
    return (
        "ZERO REVIEWED",
        'the number of jobs with status "ON REVIEW" with zero reviewed items',
    )
...
def tags_summary(stats):
    ...
    df = pd.DataFrame(
        list(
            zip(list(range(len(col_name))), col_name, col_image_count, col_object_count)
        ),
