...
class RenamingTagMetaMapper(TagMetaMapper):
    ...
    def map(self, src: TagMeta) -> TagMeta:
        ...
        return (
            self._dest_tag_meta_dict.get(dest_name, None)
            if (dest_name is not None)
            else None
        )


def make_renamed_tags(
    tags: TagCollection, tag_meta_mapper: TagMetaMapper, skip_missing=True
) -> TagCollection:
    """
    ...
    for tag in tags:
        ...
        if dest_tag_meta is not None:
            ...
            raise KeyError(
                "Tag named {} could not be mapped to a destination name.".format(
                    tag.meta.name
                )
            )
