...
def skeletonize_bitmap(
    ...
    Args:
        ...
        method_id: Algorithm of processing. See supervisely_lib.geometry.bitmap.SkeletonizeMethod enum.
    Returns:
...
def approximate_vector(
    ann: Annotation, classes: List[str], epsilon: float
) -> Annotation:
    """
...
def bitwise_mask(
    ...
    if mask_label is not None:
        target_original, target_mask = (
            mask_label.geometry.origin,
            mask_label.geometry.data,
        )
        ...
        def perform_op(label):
            if (
                label.obj_class.name not in classes_to_correct
                or label.obj_class.name == class_mask
            ):
                return [label]
...
def find_contours(
    ann: Annotation, classes_mapping: dict
    ...
    def to_contours(label: Label):
        ...
        return [
            Label(geometry=geom, obj_class=new_obj_cls)
            for geom in label.geometry.to_contours()
        ]
...
def extract_labels_from_mask(
    mask: np.ndarray, color_id_to_obj_class: collections.Mapping
) -> list:
    """
    ...
    for object_index, slices in enumerate(objects_slices, start=1):
        ...
        if class_index in color_id_to_obj_class:
            ...
            label = Label(
                geometry=bitmap, obj_class=color_id_to_obj_class.get(class_index)
            )
            labels.append(label)
    return labels
