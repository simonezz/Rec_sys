...
def is_name_included(name, enabled_names):
    return (enabled_names == MATCH_ALL) or (
        type(enabled_names) != str and name in enabled_names
    )
...
class Renamer:
    ...
    def rename(self, name):
        ...
        """
        return (
            (name + self._add_suffix)
            if is_name_included(name, self._save_names)
            else None
        )
    ...
    def to_json(self):
        ...
        """
        return {
            Renamer.ADD_SUFFIX: self._add_suffix,
            Renamer.SAVE_CLASSES: self._save_names,
        }
