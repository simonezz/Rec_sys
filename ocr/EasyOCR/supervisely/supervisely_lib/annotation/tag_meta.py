...
class TagMeta(KeyObject, JsonSerializable):
    ...
    def possible_values(self):
        return (
            self._possible_values.copy() if self._possible_values is not None else None
        )
    ...
    def from_json(cls, data):
        ...
        if isinstance(data, str):
            ...
            if color is not None:
                color = hex2rgb(color)
            return cls(
                name=name, value_type=value_type, possible_values=values, color=color
            )
        else:
    ...
    def add_possible_value(self, value):
        ...
        if self.value_type is TagValueType.ONEOF_STRING:
            if value in self._possible_values:
                raise ValueError(
                    "Value {} already exists for tag {}".format(value, self.name)
                )
            else:
    ...
    def is_valid_value(self, value):
        ...
        if self.value_type == TagValueType.NONE:
            ...
            raise ValueError(
                "Unsupported TagValueType detected ({})!".format(self.value_type)
            )
    ...
    def __str__(self):
        return "{:<7s}{:<24} {:<7s}{:<13} {:<13s}{:<10}".format(
            "Name:",
            self.name,
            "Value type:",
            self.value_type,
