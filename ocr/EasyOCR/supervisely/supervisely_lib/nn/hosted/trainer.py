...
class TrainCheckpoints:
    ...
    def saved(self, is_best, sizeb, optional_data=None):
        ...
        report_checkpoint_saved(
            checkpoint_idx=self._idx,
            subdir=self._subdir,
            sizeb=sizeb,
            best_now=report_best,
...
class SuperviselyModelTrainer:
    ...
    def _determine_model_classes_segmentation(self, bkg_input_idx):
        ...
        ) = class_indexing.create_segmentation_classes(
    ...
    def _determine_model_classes_detection(self):
        in_project_classes = self.project.meta.obj_classes
        self.out_classes = class_indexing.make_out_classes(
            in_project_classes, geometry_type=Rectangle
        )
        logger.info(
            "Determined model out classes", extra={"classes": list(self.out_classes)}
        )
    ...
    def _construct_samples_dct(self):
        ...
        self._deprecated_samples_by_tag = samples_by_tags(
            required_tags=list(self.name_to_tag.values()), project=self.project
        )
    ...
    def _save_model_snapshot(self, is_best, opt_data):
        ...
        dump_json_file(
            self.out_config, os.path.join(out_dir, TaskPaths.MODEL_CONFIG_NAME)
        )
