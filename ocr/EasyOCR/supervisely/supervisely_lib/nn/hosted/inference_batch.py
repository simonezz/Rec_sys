...
from supervisely_lib.nn.hosted.legacy.inference_config import (
...
class BatchInferenceApplier:
    ...
    def __init__(
        self,
        single_image_inference: SingleImageInferenceBase,
    ...
    def run_inference(self):
        inference_mode = InferenceModeFactory.create(
            ...
            self._in_project.meta,
            self._single_image_inference,
        )
        out_project = Project(
            os.path.join(TaskPaths.RESULTS_DIR, self._in_project.name), OpenMode.CREATE
        )
        ...
        for in_dataset in self._in_project:
            ...
            for in_item_name in in_dataset:
                ...
                in_ann = Annotation.load_json_file(
                    in_item_paths.ann_path, inference_mode.out_meta
                )
                logger.trace(
                    "Will process image",
                    extra={"dataset_name": in_dataset.name, "image_name": in_item_name},
                )
                inference_annotation = inference_mode.infer_annotate_image_file(
                    in_item_paths.img_path, in_ann
                )
