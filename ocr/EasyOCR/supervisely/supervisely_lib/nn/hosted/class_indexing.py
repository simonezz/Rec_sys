...
def make_out_classes(
    in_project_classes, geometry_type, exclude_titles=None, extra_classes=None
):
    ...
    for in_class in in_project_classes:
        if in_class.name not in exclude_titles:
            out_classes = out_classes.add(
                ObjClass(in_class.name, geometry_type, in_class.color)
            )
...
def create_segmentation_classes(
    in_project_classes,
    special_classes_config,
    bkg_input_idx,
    weights_init_type,
    model_config_fpath,
    ...
    logger.info(
        "Determined model out classes", extra={"out_classes": list(out_classes)}
    )

    in_project_class_to_idx = make_new_class_to_idx_map(
        in_project_classes,
        start_class_id=start_class_id,
        preset_class_ids=special_class_ids,
        exclude_titles=exclude_titles,
    )
    class_title_to_idx = infer_training_class_to_idx_map(
        weights_init_type,
    ...
    logger.info(
        "Determined class mapping.", extra={"class_mapping": class_title_to_idx}
    )
    return out_classes, class_title_to_idx


def make_new_class_to_idx_map(
    in_project_classes: ObjClassCollection,
    start_class_id: int,
    preset_class_ids: dict = None,
    ...
    if len(sorted_titles) != len(set(sorted_titles)):
        raise RuntimeError(
            "Unable to construct class name to integer id map: class names are not unique."
        )
...
def read_validate_model_class_to_idx_map(
    model_config_fpath, in_project_classes_set, class_to_idx_config_key
):
    ...
    if not fs.file_exists(model_config_fpath):
        raise RuntimeError(
            "Unable to continue_training, config for previous training wasn't found."
        )
    ...
    if model_class_mapping is None:
        raise RuntimeError(
            "Unable to continue_training, model does not have class mapping information."
        )
    ...
    if model_classes_set != in_project_classes_set:
        ...
        logger.critical(
            error_message_text,
            extra={
                "model_classes": model_classes_set,
                "dataset_classes": in_project_classes_set,
            },
        )
...
def infer_training_class_to_idx_map(
    ...
    if weights_init_type == TRANSFER_LEARNING:
        ...
        logger.info(
            "Continued training mode, reusing the existing class mapping from the model."
        )
    ...
    if special_class_ids is not None:
        for class_title, requested_class_id in special_class_ids.items():
            ...
            if requested_class_id != effective_class_id:
                error_msg = (
                    "Unable to start training. Effective integer id for class {} does not match the "
                ...
                logger.critical(
                    error_msg,
                    extra={
                        "class_title_to_idx": class_title_to_idx,
                        "special_class_ids": special_class_ids,
                    },
                )
