...
from supervisely_lib.annotation.obj_class_collection import (
    make_renamed_classes,
    ObjClassCollection,
)
from supervisely_lib.annotation.obj_class_mapper import (
    ObjClassMapper,
...
from supervisely_lib.annotation.tag_meta_mapper import (
...
from supervisely_lib.nn.config import (
    update_recursively,
    update_strict,
    MultiTypeValidator,
)
...
def _replace_or_drop_labels_classes(
    labels, obj_class_mapper: ObjClassMapper, tag_meta_mapper: TagMetaMapper
) -> list:
    result = []
    for label in labels:
        dest_obj_class = obj_class_mapper.map(label.obj_class)
        if dest_obj_class is not None:
            renamed_tags = make_renamed_tags(
                tags=label.tags, tag_meta_mapper=tag_meta_mapper, skip_missing=True
            )
...
def _make_cropped_rectangle(img_shape, crop_config):
    return _rectangle_from_cropping_or_padding_bounds(
        img_shape, crop_config, do_crop=True
    )


def _make_padded_rectangle(img_shape, pad_config):
    return _rectangle_from_cropping_or_padding_bounds(
        img_shape, pad_config, do_crop=False
    )
...
def _maybe_make_intermediate_bbox_class(save_config):
    return (
        None
        if not save_config.get(SAVE, False)
        else ObjClass(name=save_config[CLASS_NAME], geometry_type=Rectangle)
    )


def _maybe_make_bbox_label(roi: Rectangle, bbox_class: ObjClass, tags=None) -> list:
    return (
        [Label(geometry=roi, obj_class=bbox_class, tags=tags)]
...
class InferenceModeBase:
    ...
    def make_default_config(cls, model_result_suffix):
        return {
            MODEL_CLASSES: Renamer(
                add_suffix=model_result_suffix, save_names=MATCH_ALL
            ).to_json(),
            MODEL_TAGS: Renamer(
                add_suffix=model_result_suffix, save_names=MATCH_ALL
            ).to_json(),
    ...
    def __init__(
        self, config: dict, in_meta: ProjectMeta, model: SingleImageInferenceBase
    ):
        ...
        self._out_meta = self.out_meta.add_obj_classes(
            make_renamed_classes(
                model_out_meta.obj_classes, renamer_model, skip_missing=True
            )
        )
        ...
        # classes.
        self._model_class_mapper = RenamingObjClassMapper(
            dest_obj_classes=self._out_meta.obj_classes, renamer=renamer_model
        )
        ...
        self._model_tag_meta_mapper = RenamingTagMetaMapper(
            dest_tag_meta_dict=self._renamed_model_tags,
    ...
    def _do_infer_annotate_image_file(
        self, image_file: str, ann: Annotation
    ) -> Annotation:
...
class InfModeFullImage(InferenceModeBase):
    ...
    def _do_infer_annotate_generic(self, inference_fn, img, ann: Annotation):
        ...
        renamed_tags = make_renamed_tags(
            inference_ann.img_tags, self._model_tag_meta_mapper, skip_missing=True
        )
    ...
    def _do_infer_annotate_image_file(
        ...
        return self._do_infer_annotate_generic(
            self._model.inference_image_file, image_file, ann
        )
...
class InfModeRoi(InferenceModeBase):
    ...
    def __init__(
        ...
        self._intermediate_bbox_class = _maybe_make_intermediate_bbox_class(
            self._config
        )
    ...
    def _do_infer_annotate(self, img: np.ndarray, ann: Annotation) -> Annotation:
        ...
        img_level_tags = make_renamed_tags(
            roi_ann.img_tags, self._model_tag_meta_mapper, skip_missing=True
        )
        result_ann = result_ann.add_labels(
            _maybe_make_bbox_label(
                roi, self._intermediate_bbox_class, tags=img_level_tags
            )
        )
...
class InfModeBboxes(InferenceModeBase):
    ...
    def __init__(
        ...
        if self._config[SAVE]:
            ...
            intermediate_renamed_rectangle_classes = ObjClassCollection(
                items=[
    ...
    def _do_infer_annotate(self, img: np.ndarray, ann: Annotation) -> Annotation:
        ...
        return ann.clone(
            labels=result_labels, pixelwise_scores_labels=result_problabels
        )
...
class InfModeSlidinglWindowBase(InferenceModeBase):
    ...
    @classmethod
    def make_default_config(cls, model_result_suffix: str) -> dict:
        config = super(InfModeSlidinglWindowBase, cls).make_default_config(
            model_result_suffix
        )
        our_config = {
            WINDOW: {
...
class InfModeSlidingWindowSegmentation(InfModeSlidinglWindowBase):
    ...
    def _do_infer_annotate(self, img: np.ndarray, ann: Annotation) -> Annotation:
        ...
        for roi in self._sliding_windows.get(ann.img_size):
            ...
            model_img_level_tags = make_renamed_tags(
                raw_roi_ann.img_tags, self._model_tag_meta_mapper, make_renamed_tags
            )
            result_ann = result_ann.add_labels(
                _maybe_make_bbox_label(
                    roi, self._intermediate_bbox_class, tags=model_img_level_tags
                )
            )
        model_class_name_to_id = {
            name: idx
            for idx, name in enumerate(
                set(label.obj_class.name for label in all_pixelwise_scores_labels)
            )
        }
        id_to_class_obj = {
        ...
        if self._config.get(SAVE_PROBABILITIES, False) is True:
            ...
            accumulated_pixelwise_scores_labels = (
                raw_to_labels.segmentation_scores_to_per_class_labels(
...
class InfModeSlidingWindowDetection(InfModeSlidinglWindowBase):
    ...
    @classmethod
    def make_default_config(cls, model_result_suffix: str) -> dict:
        config = super(InfModeSlidingWindowDetection, cls).make_default_config(
            model_result_suffix
        )
        our_config = {
            NMS_AFTER: {
    ...
    def _single_class_nms(cls, labels, iou_thresh, confidence_tag_name):
        ...
        sorted_labels = sorted(
            labels, key=lambda x: x.tags.get(confidence_tag_name).value
        )
        return [
            label
            for label_idx, label in enumerate(sorted_labels)
            if all(
                cls._iou(label.geometry.to_bbox(), other.geometry.to_bbox())
                <= iou_thresh
    ...
    def _general_nms(cls, labels, iou_thresh, confidence_tag_name):
        if not all(isinstance(label.geometry, Rectangle) for label in labels):
            raise RuntimeError(
                "Non-max suppression expects labels with Rectangle geometry."
            )
        if not all(label.tags.has_key(confidence_tag_name) for label in labels):
            raise RuntimeError(
                'Non-max suppression expects "{}" tag in labels.'.format(
                    confidence_tag_name
                )
            )
        if not all(
            label.tags.get(confidence_tag_name).meta.value_type
            == TagValueType.ANY_NUMBER
            for label in labels
        ):
            raise RuntimeError(
                'Non-max suppression expects "{}" tag to have type {}.'.format(
                    confidence_tag_name, TagValueType.ANY_NUMBER
                )
            )
        ...
        survived_labels = [
            cls._single_class_nms(single_class_labels, iou_thresh, confidence_tag_name)
            for single_class_labels in labels_per_class.values()
        ]
    ...
    def _do_infer_annotate(self, img: np.ndarray, ann: Annotation) -> Annotation:
        ...
        for roi in self._sliding_windows.get(ann.img_size):
            ...
            model_labels.extend(
                [
                    label
                    for label in raw_roi_ann.labels
...
class InferenceModeFactory:
    mapping = {
        inference_mode_cls.mode_name(): inference_mode_cls
        for inference_mode_cls in [
            InfModeFullImage,
            InfModeRoi,
            InfModeBboxes,
...
def get_effective_inference_mode_config(
    task_inference_mode_config: dict, default_inference_mode_config: dict
) -> dict:
