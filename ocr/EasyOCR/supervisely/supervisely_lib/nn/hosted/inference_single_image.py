...
from supervisely_lib.nn.hosted.constants import (
    MODEL,
    SETTINGS,
    INPUT_SIZE,
    HEIGHT,
...
class SingleImageInferenceBase(SingleImageInferenceInterface):
    def __init__(self, task_model_config=None, _load_model_weights=True):
        ...
        task_model_config = (
            ...
            else deepcopy(task_model_config)
        )
    ...
    def _load_raw_model_config_json(self):
        try:
            ...
            raise RuntimeError(
                "Unable to run inference, config from training was not found."
            )
    ...
    def _load_train_config(self):
        ...
        logger.info(
            "Read model internal class mapping",
        ...
        logger.info(
            "Read model out classes", extra={"classes": train_classes.to_json()}
        )

        # TODO: Factor out meta constructing from _load_train_config method.
        self._model_out_meta = ProjectMeta(
            obj_classes=train_classes, tag_metas=self._model_out_tags()
        )
    ...
    def _determine_model_input_size(self):
        ...
        logger.info(
            ...
            extra={INPUT_SIZE: {WIDTH: self.input_size[1], HEIGHT: self.input_size[0]}},
        )
