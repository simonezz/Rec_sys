...
class PytorchSegmentationApplier(SingleImageInferenceBase):
    ...
    def _construct_and_fill_model(self):
        ...
        self._model = self._model_factory_fn(
            ...
            custom_model_config=self.train_config[SETTINGS].get(
                CUSTOM_MODEL_CONFIG, {}
            ),
        )
    ...
    def inference(self, img, ann):
        ...
        # scores space.
        class_scores_original_size = sly.image.resize_inter_nearest(
            class_scores_hwc, out_size=img.shape[:2]
        )
        ...
        labels = raw_to_labels.segmentation_array_to_sly_bitmaps(
            idx_to_class=self.out_class_mapping, pred=class_ids
        )
