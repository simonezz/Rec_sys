...
from supervisely_lib.nn.hosted.class_indexing import (
    CONTINUE_TRAINING,
    TRANSFER_LEARNING,
)
from supervisely_lib.nn.hosted.pytorch.constants import CUSTOM_MODEL_CONFIG, HEAD_LAYER
from supervisely_lib.nn.hosted.trainer import (
...
from supervisely_lib.task.progress import (
    Progress,
    epoch_float,
...
def _check_all_pixels_have_segmentation_class(targets):
    if targets.min().item() < 0:
        raise ValueError(
            "Training data has an item that is not fully covered by the segmentation labeling "
...
class PytorchSegmentationTrainer(SuperviselyModelTrainer):
    ...
    def __init__(
        self,
        model_factory_fn,
        optimization_loss_fn,
        training_metrics_dict=None,
        default_model_config=None,
    ):
        default_config = PytorchSegmentationTrainer.get_default_config()
        default_config[CUSTOM_MODEL_CONFIG].update(
            default_model_config if default_model_config is not None else {}
        )

        self._model_factory_fn = model_factory_fn
        self._optimization_loss_fn = optimization_loss_fn
        self._training_metrics_dict = (
            training_metrics_dict.copy() if training_metrics_dict is not None else {}
        )
    ...
    def _construct_and_fill_model(self):
        ...
        logger.info(
            "Weights have been loaded.", extra={WEIGHTS_INIT_TYPE: weights_init_type}
        )
    ...
    def _construct_data_loaders(self):
        ...
        for dataset_name, need_shuffle, drop_last in [
            (TRAIN, True, True),
            (VAL, False, False),
        ]:
        ...
        self._eval_planner = EvalPlanner(
            epochs=self._epochs, val_every=self.config[VAL_EVERY]
        )
    ...
    def train(self):
        ...
        training_progress = Progress(
            "Model training: ", self._epochs * self._train_iters
        )
        ...
        for epoch in range(self._epochs):
            ...
            for train_it, (inputs_cpu, targets_cpu) in enumerate(
                ...
                inputs_cuda, targets_cuda = (
                    Variable(inputs_cpu).cuda(),
                    Variable(targets_cpu).cuda(),
                )
                outputs_cuda = self._model(inputs_cuda)
