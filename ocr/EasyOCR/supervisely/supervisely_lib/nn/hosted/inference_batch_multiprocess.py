...
from supervisely_lib.nn.hosted.inference_batch import (
    determine_task_inference_mode_config,
)
from supervisely_lib.task.progress import Progress


InferenceRequest = namedtuple(
    "InferenceRequest", ["ds_name", "item_name", "item_paths"]
)
InferenceResponse = namedtuple(
    "InferenceResponse", ["ds_name", "item_name", "item_paths", "ann_json", "meta_json"]
)


def single_inference_process_fn(
    inference_initializer,
    inference_mode_config,
    in_project_meta_json,
    request_queue,
    result_meta_queue,
    progress_queue,
    project,
    ...
    inference_mode = InferenceModeFactory.create(
        inference_mode_config,
        ProjectMeta.from_json(in_project_meta_json),
    ...
    while req is not None:
        ...
        if req is not None:
            ...
            if not project_meta_sent:
                try:
                    result_meta_queue.put(
                        inference_mode.out_meta.to_json(), block=False
                    )
                except queue.Full:
                    pass
            project_meta_sent = True

            in_ann = Annotation.load_json_file(
                req.item_paths.ann_path, inference_mode.out_meta
            )
...
class BatchInferenceMultiprocessApplier:
    ...
    def __init__(
        ...
        logger.info(
            "Project structure has been read. Samples: {}.".format(
        ...
        self._inference_request_queue = mp.Queue(
            maxsize=self.QUEUE_ELEMENTS_PER_PROCESS * num_processes
        )
        self._result_meta_queue = mp.Queue(maxsize=3)
        self._progress_report_queue = mp.Queue(
            maxsize=self.QUEUE_ELEMENTS_PER_PROCESS * num_processes
        )
    ...
    def run_inference(self):
        progress_report_thread = Thread(
            target=progress_report_thread_fn,
            args=(self._in_project, self._progress_report_queue),
            daemon=True,
        )
        ...
        if not feed_status or not all(
            p.exitcode == 0 for p in self._inference_processes
        ):
