...
class RestInferenceServer:
    def __init__(self, model: SingleImageInferenceInterface, name, port=None):
        ...
        api = Api(self._app)
        api.add_resource(
            RestInferenceServer.GetOutputMeta,
            "/" + MODEL + "/" + GET_OUTPUT_META,
            resource_class_kwargs={"model": model},
        )
        api.add_resource(
            RestInferenceServer.Inference,
            "/" + MODEL + "/" + INFERENCE,
            resource_class_kwargs={"model": model},
        )
    ...
    class Inference(Resource):
        ...
        def post(self):
            ...
            data = {
                ...
                "meta": json.loads(meta.stream.read().decode("utf-8"))
                if meta is not None
                else ProjectMeta().to_json(),
                "annotation": json.loads(ann.stream.read().decode("utf-8"))
                if ann is not None
                else None,
                "mode": json.loads(mode.stream.read().decode("utf-8"))
                if mode is not None
                else {},
                "image_arr": img,
            }
