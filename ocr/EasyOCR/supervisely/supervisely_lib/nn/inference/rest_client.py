...
from supervisely_lib.nn.inference.rest_constants import (
    GET_OUTPUT_META,
    IMAGE,
    INFERENCE,
    MODEL,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Inference REST client for standalone Supervisely models."
    )
    parser.add_argument("--server-url", required=True)
    parser.add_argument(
        "--request-type", required=True, choices=SUPPORTED_REQUEST_TYPES
    )
    ...
    if args.request_type == GET_OUTPUT_META:
        ...
        encoder = MultipartEncoder(
            {IMAGE: (args.in_image, io.BytesIO(img_bytes), "application/octet-stream")}
        )
        response = requests.post(
            request_url, data=encoder, headers={"Content-Type": encoder.content_type}
        )
    else:
        raise ValueError(
