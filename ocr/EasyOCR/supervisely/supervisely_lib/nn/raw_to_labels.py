...
def segmentation_scores_to_per_class_labels(
    ...
    return [
        PixelwiseScoresLabel(
            geometry=MultichannelBitmap(
                data=segmentation_scores[:, :, cls_idx, np.newaxis]
            ),
            obj_class=cls_obj,
        )
        for cls_idx, cls_obj in idx_to_class.items()
...
DetectionNetworkPrediction = namedtuple(
    "DetectionNetworkPrediction", ["boxes", "scores", "classes"]
)
