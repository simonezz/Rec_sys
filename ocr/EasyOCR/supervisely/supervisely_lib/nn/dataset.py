...
def samples_by_tags(required_tags, project):
    ...
    for dataset in project:
        for item_name in dataset:
            ...
            ann = Annotation.load_json_file(
                path=item_paths.ann_path, project_meta=project.meta
            )
...
class SlyDataset:
    ...
    def __getitem__(self, index):
        for att in range(self._catcher_retries):
            ...
            res = self._sample_catcher.exec(
                index,
                {"img": img_path, "ann": ann_path},
...
def partition_train_val(num_samples, val_fraction):
    ...
    if num_samples <= 1:
        raise ValueError(
            "Need at least 2 samples to prepare a training/validation split (at least 1 each for training "
            "and validation)."
        )
