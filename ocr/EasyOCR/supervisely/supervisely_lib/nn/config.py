...
class MultiTypeValidator(object):
    ...
    def __init__(self, schema_fpath):
        ...
        self._validators = {
            name: vtor_class(schema) for name, schema in schemas.items()
        }
    ...
    def val(self, type_name, obj):
        ...
        if validator is None:
            raise RuntimeError(
                "JSON validator is not defined. Type: {}".format(type_name)
            )
        try:
            validator.validate(obj)
        except Exception as e:
            raise RuntimeError(
                "Error occurred during JSON validation. Type: {}. Exc: {}. See documentation".format(
                    type_name, str(e)
                )
...
def update_strict(lhs, rhs):
    for k, v in rhs.items():
        if k in lhs:
            raise ValueError(
                'Key collision while strict updating. Key:"{}"'.format(str(k))
            )
        lhs[k] = v
    return lhs
