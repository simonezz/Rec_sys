...
class WeightsRW:
                ...
                try:
                    ...
                    raise RuntimeError(
                        ...
                        "whose dimensions in the checkpoint are {}.".format(
                            name, dest_state[name].size(), param.size()
                        )
                    )

    def load_for_transfer_learning(
        self, model, ignore_matching_layers=None, logger=None
    ):
        ...
        to_delete = set(
            el
            for el in snapshot_weights.keys()
            if any(
                delete_substring in el for delete_substring in ignore_matching_layers
            )
        )
        snapshot_weights = {
            k: v for k, v in snapshot_weights.items() if k not in to_delete
        }
        if len(to_delete) > 0 and logger is not None:
            logger.info(
                "Skip weight init for output layers.",
                extra={"layer_names": sorted(to_delete)},
            )
    ...
    def load_strictly(self, model):
        ...
        if len(snapshot_extra_keys) > 0:
            raise KeyError(
                "Parameters found in the snapshot file, but not in the model: {}".format(
                    snapshot_extra_keys
                )
            )
        ...
        if len(model_extra_keys) > 0:
            raise KeyError(
                "Model parameters missing from the snapshot file: {}".format(
                    model_extra_keys
                )
            )
