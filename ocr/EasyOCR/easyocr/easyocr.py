...
class Reader(object):
    def __init__(
        ...
        if recog_network != "standard":
            with open(
                os.path.join(self.user_network_directory, recog_network + ".yaml")
            ) as file:
            ...
            self.setModelLanguage(
                recog_network, lang_list, available_lang, available_lang
            )
            char_file = os.path.join(
                self.user_network_directory, recog_network + ".txt"
            )
            ...
            if "th" in lang_list:
                ...
                self.setModelLanguage(
                    "chinese_tra", lang_list, ["ch_tra", "en"], '["ch_tra","en"]'
                )
            elif "ch_sim" in lang_list:
                self.setModelLanguage(
                    "chinese_sim", lang_list, ["ch_sim", "en"], '["ch_sim","en"]'
                )
            elif "ja" in lang_list:
                self.setModelLanguage(
                    "japanese", lang_list, ["ja", "en"], '["ja","en"]'
                )
                ...
                self.setModelLanguage(
                    "bengali", lang_list, bengali_lang_list + ["en"], '["bn","as","en"]'
                )
            ...
            if self.model_lang == "latin":
                ...
                self.character = (
                    ...
                    + symbol
                    + characters["en_char"]
                    + characters["ar_number"]
                    + characters["ar_symbol"]
                    + characters["ar_char"]
                )
                ...
                self.character = (
                    ...
                    + symbol
                    + characters["en_char"]
                    + characters["devanagari_char"]
                )
                model_file = "devanagari.pth"
            elif self.model_lang == "bengali":
                self.character = (
                    number + symbol + characters["en_char"] + characters["bn_char"]
                )
                ...
                self.character = (
                    number + symbol + characters["en_char"] + characters["te_char"]
                )
                model_file = "telugu.pth"
                recog_network = "lite"
            elif self.model_lang == "kannada":
                self.character = (
                    number + symbol + characters["en_char"] + characters["kn_char"]
                )
            ...
            if os.path.isfile(model_path) == False:
                ...
                download_and_unzip(
                    model_url[model_file][0], model_file, self.model_storage_directory
                )
                assert (
                    calculate_md5(model_path) == model_url[model_file][1]
                ), corrupt_msg
                LOGGER.info("Download complete.")
    ...
    def setModelLanguage(self, language, lang_list, list_lang, list_lang_string):
        ...
        if set(lang_list) - set(list_lang) != set():
            ...
            raise ValueError(
                language.capitalize()
                + " is only compatible with English, try lang_list="
                + list_lang_string
            )
    ...
    def recognize(
        ...
        filter_ths=0.003,
        reformat=True,
        enable_mathpix_=False,
        convert_plain_=False,
    ):
        ...
        if (horizontal_list == None) and (free_list == None):
            ...
            crop_img = cv2.resize(
                img_cv_grey, (max_width, imgH), interpolation=Image.ANTIALIAS
            )
