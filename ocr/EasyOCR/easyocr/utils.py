...
def group_text_box_by_ths(ocr_results, ycenter_ths=0.5, height_ths=0.5, width_ths=1.0):
    # poly top-left, top-right, low-right, low-left
    combined_boxes, combined_texts, combined_scores = [], [], []
    merged_results = []

    sort_results = swap_text_positions(
        ocr_results, axis=0, y_margin=40
    )  # axis=0(row), 1(col)
    ...
    new_text = []
    new_score = []
    for i, (box, text, score) in enumerate(sort_results):
        box = coord.convert_rect4_to_rect2(box)
        box = [box[0], box[1], box[2], box[3]]

        if len(new_box) == 0:
            b_height = [box[3] - box[2]]  # poly[5]
            b_ycenter = [0.5 * (box[2] + box[3])]  # poly[4]
            new_box.append(box)
            ...
            # comparable height and comparable y_center level up to ths*height
            if (
                abs(np.mean(b_height) - (box[3] - box[2]))
                < height_ths * np.mean(b_height)
            ) and (
                abs(np.mean(b_ycenter) - (0.5 * (box[2] + box[3])))
                < ycenter_ths * np.mean(b_height)
            ):
                b_height.append(box[3] - box[2])
                b_ycenter.append(0.5 * (box[2] + box[3]))
                new_box.append(box)
                new_text.append(text)
                new_score.append(score)
            else:
                b_height = [box[3] - box[2]]
                b_ycenter = [0.5 * (box[2] + box[3])]
                combined_boxes.append(new_box)
                combined_texts.append(new_text)
    ...
    # Merge by width_ths
    for boxes, texts, scores in zip(combined_boxes, combined_texts, combined_scores):
        elements = sorted(list(zip(boxes, texts, scores)), key=lambda item: item[0][0])

        new_box, new_text, new_score = [], [], []
        merged_box, merged_text, merged_score = [], [], []
        for box, text, score in elements:
            if len(new_box) == 0:
                x_max = box[1]
                new_box.append(box)
                new_text.append(text)
                ...
                if abs(box[0] - x_max) < width_ths * (box[3] - box[2]):  # merge boxes
                    ...
                    x_max = box[1]
                    merged_box.append(new_box)
                    merged_text.append("".join(new_text))
                    merged_score.append(sum(new_score) / len(new_score))
                    new_box = [box]
                    new_text = [text]
                    new_score = [score]
        ...
        for mbox, mtext, mscore in zip(merged_box, merged_text, merged_score):
            if len(mbox) != 1:  # adjacent box in same line
                # do I need to add margin here?
                x_min = min(mbox, key=lambda x: x[0])[0]
                x_max = max(mbox, key=lambda x: x[1])[1]
                y_min = min(mbox, key=lambda x: x[2])[2]
                y_max = max(mbox, key=lambda x: x[3])[3]
                merge_box = [int(x_min), int(x_max), int(y_min), int(y_max)]

                rst_box = coord.convert_rect2_to_rect4(merge_box)
                rst_text = mtext
                rst_score = mscore

                merged_results.append([rst_box, rst_text, rst_score])
            else:  # non adjacent box in same line
                box = mbox[0]
                merge_box = [int(box[0]), int(box[1]), int(box[2]), int(box[3])]
                rst_box = coord.convert_rect2_to_rect4(merge_box)
                rst_text = mtext
                rst_score = mscore
...
def swap_text_positions(text_infos, axis=0, y_margin=40):
    if axis == 0:
        for i in range(0, len(text_infos) - 1):
            # rt_x, next_lt_x = text_infos[i][0][1][0], text_infos[i+1][0][0][0] # right-top x, left-top x
            c_x, next_c_x = int(
                (text_infos[i][0][0][0] + text_infos[i][0][1][0]) / 2
            ), int(
                (text_infos[i + 1][0][0][0] + text_infos[i + 1][0][1][0]) / 2
            )  # center x
            y, next_y = text_infos[i][0][0][1], text_infos[i + 1][0][0][1]
            # x_diff_ = (abs(rt_x - next_lt_x) < x_margin) and (rt_x > next_lt_x)
            y_diff_ = abs(y - next_y) < y_margin
            x_center_ = c_x > next_c_x
            if (y_diff_) and (x_center_):
                text_infos[i], text_infos[i + 1] = (
                    text_infos[i + 1],
                    text_infos[i],
                )  # swap row index
